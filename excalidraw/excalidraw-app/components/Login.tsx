...
export const Login = ({ onAuthenticated }: Props) => {
	...
	const handleLogin = async () => {
		...
		try {
			...
			interface EmployeeJoinRow { company_id: string; companies: { name: string } }
			...
			const employee = (employeeRows as unknown as EmployeeJoinRow[] | null)?.[0];
			...
			<div style={styles.panel}>
				...
				<div style={styles.switchMode}>
					{mode === "login" ? (
						<>
							...
							<button style={styles.linkBtn} onClick={() => setMode("signup")}>Sign up</button>
						</>
					) : (
						<>
							<span>Already have an account?</span>{" "}
							<button style={styles.linkBtn} onClick={() => setMode("login")}>Login</button>
						</>
					)}
				</div>
				...
				{!supabase && (
					<div style={styles.error}>
						Supabase client not initialized. Add VITE_SUPABASE_URL & VITE_SUPABASE_ANON_KEY.
					</div>
				)}
			</div>
		</div>
